...
import random
import sys
from tools.doubao_client import DoubaoClient
...
class Simulator:
    def __init__(self, api_key: str, base_url: str, model: str, community_token: str, community_base_url: str, db_config: dict | None = None, energy: int = 50):
        ...
        self.client = DoubaoClient(api_key, base_url)
        self.model = model
    ...
    def _generate_meta_prompt(self, page_name: str):
        ...
        return f"""
            ...
            如果你已经存在了某些记忆，请根据记忆从阅读进度开始继续阅读，**而不是**重新阅读已经读过的章节。如果你已经读过某些内容但没有关于它的记忆，你可能需要重新阅读。
            每读完一篇文章，你被鼓励创建关于它的记忆，以便下次获取。
    ...
    def start(self):
        ...
        while True:
            try:
                ...
                # 获取AI响应
                response = self.client.chat(page_info, model=self.model, use_search=True, use_deep_think=True, use_auto_cot=False)
...
def main():
    ...
    doubao_api = "http://8.152.99.246:9005/"
    doubao_api_key = ""

    simulator = Simulator(doubao_api_key, doubao_api, "doubao", community_token, "https://loghomeservice.codesocean.top", db_config={
