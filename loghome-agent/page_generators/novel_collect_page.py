...
def generate_page(token: str, base_url: str, params: dict, memory_manager: MemoryManager | None = None, reading_record_manager=None):
    ...
    try:
        # 调用收藏接口
        collect_response = requests.post(f"{base_url}/bookcase/like_novel", data={"novel_id": novel_id}, headers=headers)
        ...
        if collect_response.status_code == 200:
            ...
            return f"""
            你成功收藏了小说《{novel_name}》（ID：{novel_id}）！
            """
        else:
            print(collect_response)
            return f"""
            收藏小说失败，错误代码：{collect_response.status_code}
            """
