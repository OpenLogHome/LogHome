...
def generate_page(token: str, base_url: str, params: dict, memory_manager: MemoryManager | None = None, reading_record_manager=None):
    ...
    try:
        # 调用取消收藏接口
        uncollect_response = requests.post(f"{base_url}/bookcase/remove_like_novel", data={"novel_id": novel_id}, headers=headers)
        ...
        if uncollect_response.status_code == 200:
            ...
            return f"""
            你成功取消收藏了小说《{novel_name}》（ID：{novel_id}）。
            """
        else:
            print(uncollect_response)
            return f"""
            取消收藏小说失败，错误代码：{uncollect_response.status_code}
            """
