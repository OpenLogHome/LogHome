	...
	try {
		...
		setTimeout(() => {
			try {
				...
				// 填充虚拟页
				page2.innerHTML = pageToHideContent;
				...
				if (!enhanced) {
					...
					if (item === 'navigateBack') {
						return new Promise(function(resolve, reject){
							const uniPage = document.querySelector('uni-page');
							pageToHideContent = uniPage.innerHTML;
							resolve(e);
						});
					}
	...
	if(auto){
		window.addEventListener("popstate",(e)=>{
			...
			if (!enhanced) {
				...
				// 使用增强版返回动画
				if(pageToHideContent === null) {
					pageToHideContent = lastPageContent;
				}

				enhancedPageHide(() => {
					pageToHideContent = null;
					setTimeout(() => {
						const page1_class = document.querySelector('uni-page')?.classList;
						if (page1_class) {
							page1_class.add('animation-show');
