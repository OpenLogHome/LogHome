...
<!-- 	<el-alert
	...
	<canvas id="touch" canvas-id="myCanvas" @touchstart="touchstart" @touchmove="touchmove" @touchend="touchend"></canvas>
	<view id="clipArea">
		<view class="top"></view>
	...
	<view class="tanchuang" v-show="yulanPicSrc !== ''">
		<view class="alert notitle">
			...
			<view class="bottom line"><view class="vertical_line" @click="yulanPicSrc = ''">确定</view></view>
		</view>
	</view>
</view></template>
<script>
...
export default {
    ...
    methods: {
		...
		touchmove: function(e){//手指移动
			if(anxiaY === undefined){return 
				...
				//改变css平移属性，来实现移动
					this.imgY = ((((isPx ? e.y : e.touches[0].y) - anxiaY) * heightBeishu) + yuanMarginTop);//垂直移动了多少个像素 * 移动的速度 + 图片原本定位的位置
				...
				var linshi = scale * (this.getDistance(e.touches[0],e.touches[1]) / this.getDistance(start[0],start[1]));
		...
		imgLeft_AND_imgTop: function(width, height){//【计算图片的左上定位】。有些同学看了之后就问啦！为什么不直接获取元素的布局信息，布局信息也有宽高和定位啊，而要大费周章的计算呢。那是因为直接获取元素布局信息，需要等元素的某些动画结束之后才能获取，例如，图片放小之后，缩放比例就要调整回100%并且可能还要靠边。那这时候靠边就得在缩放动画完成之后才能执行，所以两个动画就不能一起执行了。
			...
			//缩放之后，位置是不一样的，得重新计算
				left -= (Math.min(this.imgWidth, this.imgHeight) / 2) * (this.imgScale - 1)//为什么要取宽高中的最小值呢？大概率跟旋转中心点有关【没试验，猜的】
		...
		huitan: function(){
			...
			//判定是否需要上下靠边
				...
				if(typeof(x) === 'number'){
					this.imgX = x + ((Math.min(this.imgWidth, this.imgHeight) * (this.imgScale - 1)) / 2)//这里取最小值应该是和旋转中心点有关
				}
				if(typeof(y) === 'number'){
		...
		jieqv: function(){
			if(this.url === ''){this.jiazaiImg()
			}else if(this.bottomText_4 === '截取中...'){uni.showToast({title: '截取中，请稍等',icon: 'none',duration: 1500})
			}else if(this.bottomText_4 === '上传中...'){uni.showToast({title: '上传中，请稍等',icon: 'none',duration: 1500})
			}else if(this.bottomText_2 === '旋转中...'){uni.showToast({title: '旋转中，请稍等',icon: 'none',duration: 1000})
			}else{
		...
		getImageInClipAreaInfo: function(){//获取图片在裁剪区的信息
			...
			//截取的位置
				//X轴（水平）
					...
					var dx = ([0,180].indexOf(imageRotate)>-1 ? img_yuanshi_width : img_yuanshi_height) * beishuX;//计算出【基于源图像】的偏移px
					if(imageRotate === 90){
		...
		jieqvTobase64: function(fn){
			...
			ctx.draw(false, (e) => {//绘制成功【绘制到 canvas 中】【false=清空canvas之前的内容】	return 
				...
				uni.canvasToTempFilePath({//canvas保存为图片
					...
					success: (res) => {
						//#ifdef APP-PLUS
							plus.io.resolveLocalFileSystemURL(res.tempFilePath,(entry) => {//通过URL参数获取目录对象或文件对象
								entry.file((file) => {
									var fileReader = new plus.io.FileReader()
									fileReader.onload = (data) => {//加载完成
										this[fn](data.target.result);
									}
									fileReader.readAsDataURL(file)//以URL编码格式读取文件数据内容【转Base64的意思】
								})
							})
						//#endif
						//#ifdef H5
		...
		yulan: function(){
			if(this.url === ''){this.jiazaiImg()
			}else if(this.bottomText_3 === '截取中...'){uni.showToast({title: '截取中，请稍等',icon: 'none',duration: 1500})
			}else if(this.bottomText_2 === '旋转中...'){uni.showToast({title: '旋转中，请稍等',icon: 'none',duration: 1000})
			}else{
	...
	onReady: function(){
		uni.getSystemInfo({success: (res)=> {
			...
			uni.createSelectorQuery().select('#clipArea').boundingClientRect((data) => {//上下左右是相对于视口的
...
@media screen and (-webkit-min-device-pixel-ratio: 2){
	.line:before{background-image: linear-gradient(to bottom,transparent 50%,#F1F1F1 0%);}
	.vertical_line:after{background-image: linear-gradient(to right,#F1F1F1 50%,transparent 50%);}
}
@media screen and (-webkit-min-device-pixel-ratio: 2.5){
	.line:before{background-image: linear-gradient(to bottom,transparent %,#F1F1F1 0%);}
	.vertical_line:after{background-image: linear-gradient(to right,#F1F1F1 55%,transparent 55%);}
}
@media screen and (-webkit-min-device-pixel-ratio: 3){
	.line:before{background-image: linear-gradient(to bottom,transparent 60%,#F1F1F1 0%);}
	.vertical_line:after{background-image: linear-gradient(to right,#F1F1F1 60%,transparent 60%);}
}
/* 弹窗的样式开始 */
.tanchuang{
	width: 100%;
...
.tanchuang > .alert .content::-webkit-scrollbar{/* 隐藏滚动条，css3属性 */
	display: none;
}
