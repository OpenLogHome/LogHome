	...
	<view style="background-color:rgb(255, 248, 234);">
		...
		<springBack :top="`calc(300rpx + ${jsBridge.inApp ? jsBridge.statusBarHeight + 'px' : 0 + 'px'})`">
			...
			<view style="display: flex;align-items: center;margin-left: 50rpx;margin-top: 28rpx;">
				<text style="font-size: 40rpx;color: #111111;font-weight: bold;margin-right: 10rpx;">{{ user.name
				}}</text>
			...
			<view class="moreInfo" style="margin-left: 50rpx;margin-top: 18rpx;">
				...
				<span class="admin_title" v-show="user.is_admin">
					<img src="../../static/icons/admin.gif" alt=""
						style="width:45rpx;margin-left: 10rpx;" />社区管理员</span>
			</view>
			...
			<view style="display: flex;align-items: center;margin-left: 50rpx;margin-top: 20rpx;margin-bottom: 20rpx;">
				<navigator :url="'../community/friends?id=' + uid + '&tab=1'">
					<text style="font-size: 40rpx;font-weight: bold;color: #555555;margin-right: 18rpx;">{{ fans
					}}</text><text style="font-size: 28rpx;color: gray;margin-right: 28rpx;">粉丝</text>
				</navigator>
				<navigator :url="'../community/friends?id=' + uid + '&tab=0'">
					<text style="font-size: 40rpx;font-weight: bold;color: #555555;margin-right: 18rpx;">{{ follows
					}}</text><text style="font-size: 28rpx;color: gray;margin-right: 28rpx;">关注</text>
				</navigator>
			...
			<view id="tabbar" :class="isFixed ? 'tabbar-fixed' : ''"
				...
				<view style="font-size: 32rpx;font-weight: bold;text-align: center;width: 128rpx;"
					:class="current == 1 ? 'tabbarsh' : 'notabbarsh'" @tap="fnBarClick(1)">世界</view>
			</view>
			<!-- 添加 scroll-view-indicator -->
			...
			<!-- 添加 swiper 组件 -->
			<swiper :current="current" class="swiper" @change="ontabchange" @transition="onswiperscroll"
				@animationfinish="endswiperscroll">
				<swiper-item>
					<div class="bookcase">
						<bookInCase v-for="item in booksOnShow" :bookName="item.name" :picUrl="item.picUrl"
							:key="item.novel_id" @click.native="readBook(item.novel_id)" v-show="!item.is_personal">
						</bookInCase>
					</div>
				</swiper-item>
				<swiper-item>
					<div class="bookcase">
						<bookInCase v-for="item in worldsOnShow" :bookName="item.name" :picUrl="item.picUrl"
							:key="item.world_id" @click.native="readBook(item.novel_id)"></bookInCase>
					</div>
				</swiper-item>
			</swiper>
		</springBack>
...
export default {
	...
	data() {
		return {
			...
			currentIndicatorLineLeft: 0,
			swiperWidth: 0,
		}
	},
	async onLoad(option) {
		this.uni_id = option.uni_id;
		this.uid = option.uid;
	},
	onReady() {
		this.initSelectorQuery(this.current);
	},
	methods: {
		/// 顶部导航选项点击
		fnBarClick(current) {
		...
		onswiperscroll(e) {
			this.indicatorLineLeft = (this.swiperWidth / 2) + this.swiperWidth * (this.current) + (this.swiperWidth /
				this.screenWidth) * e.detail.dx;
		},
		initSelectorQuery(current) {
			const that = this;
			uni.createSelectorQuery()
				.select('#tabbar > view:nth-child(' + (current + 1) + ')')
				.boundingClientRect(rect => {
					that.indicatorLineLeft = (rect.width / 2) + rect.width * (current);
					that.indicatorLineWidth = rect.width / 2;
					that.swiperWidth = rect.width;
				}).exec();
		}
	},
	async onShow() {
		if(this.uni_id != ""){
			let data = (await axios.get(this.$baseUrl + '/users/get_user_id_by_uni_id?uni_id=' + this.uni_id, {})).data;
			if(data.length > 0){
				this.uid = data[0].user_id;
			} else {
				uni.showToast({
					title: "用户不存在",
					icon: 'none',
...
.swiper {
	height: calc(100vh - 300rpx - 200rpx);
}
</style>
