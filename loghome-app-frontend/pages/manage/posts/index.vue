	...
	<view class="container">		
		<view class="content">
			...
			<view class="post-list">
				<view class="post-item" v-for="post in posts" :key="post.post_id" @click="viewPost(post)">
					<view class="post-main">
						<view class="post-title">{{post.title}}</view>
						<view class="post-info">
							<text class="author">作者: {{post.name}}</text>
						</view>
					</view>
					<view class="post-actions">
						<view class="action-btn edit" @click.stop="editPost(post)">编辑</view>
						<view class="action-btn delete" @click.stop="deletePost(post)">删除</view>
					</view>
				</view>
			</view>
		...
		<view class="modal" v-if="showModal">
			...
			<view class="modal-content">
				<view class="modal-header">
					<view class="modal-title">{{currentPost.post_id ? '编辑帖子' : '发布帖子'}}</view>
				...
				<view class="modal-body">
					<view class="form-item">
						...
						<view class="input">
							<input type="text" v-model="currentPost.title" placeholder="请输入标题" />
						</view>
					</view>
					<view class="form-item">
		...
		async editPost(post) {
			try {
				...
				if (res.data && res.data.length > 0) {
					...
					setTimeout(() => {
						if (this.editorCtx) {
							this.editorCtx.setContents({html: this.currentPost.content || ''})
						}
					}, 300)
				}
		...
		onEditorReady() {
			uni.createSelectorQuery()
				...
				.context((res) => {
					...
					if (this.currentPost.content && this.currentPost.post_id) {
						this.editorCtx.setContents({html: this.currentPost.content})
					}
				})
				.exec()
