<template>
	<div class="outer" :class="writerSettings.theme" style="transition: all .5s;">
		<div class="topBar">
			...
			<div class="textCount">{{paraCount}}&nbsp;段 {{textCount}}&nbsp;字 {{imgCount}}&nbsp;图</div>
		</div>
		<div class="middleBar" v-show="writerSettings.codeMode">
			<editor class="textarea" placeholder="" @input="onInput"
			 @ready="onEditorReady" :style="{fontSize:writerSettings.fontSize + 'rpx'}"></editor>
		</div>
		<uni-popup ref="setPopup" type="bottom" style="z-index:101">
			<view class="settingBar">
				...
				<div class="normalLine">
					...
					<div class="right"><el-slider v-model="writerSettings.fontSize" :min="31" :max="49" :step="2"
							show-stops :show-tooltip="false" @change="fontSizeChanged"></el-slider></div>
				</div>
				<div class="lrLine">
					<div class="left">源代码模式（非高级用户请勿使用）</div>
					<div class="right">
						<el-switch
						  v-model="writerSettings.codeMode"
						  active-color="#ff4949"
		...
		<customlist :movement-button="true" :text-length="5000" @on-change="change" v-show="!writerSettings.codeMode"
		ref="listEditor" :fontSize="{fontSize:writerSettings.fontSize + 'rpx'}">
		</customlist>
	</div>
</template>
		...
		methods:{
			...
			onEditorReady(){
				...
				axios.get(this.$baseUrl + '/essays/get_article?id=' + this.chapterId, 
				{
					...
					this.$refs.listEditor.reInit(this.article.content);
					this.countText();
			...
			countText(){
				let articleList = JSON.parse(this.article.content);
				let textCount = 0;
				let imgCount = 0;
				this.paraCount = articleList.length;
			...
			formatEssay(){
				this.editorCtx.getContents({
					success:(res)=>{
						let articleContent = JSON.parse(this.article.content);
						for(let item of articleContent){
							if(item.type == "text"){
								let articleTiles = item.value.split("\n");
								...
								for(let i = 0; i <articleTiles.length ; i++) {
									if(articleTiles[i] != ""){
										item.value = item.value + "　　" + articleTiles[i] + "\n";
						...
						this.article.content = JSON.stringify(articleContent);
						this.editorCtx.setContents({//赋值
							  delta:{
								  ops:[
									  {
										insert:JSON.stringify(articleContent)
									  }
								  ]
							  }
						});
														...
														uni.createSelectorQuery().select('.textarea').context((res) => {
														    ...
														    _this.editorCtx.setContents({//赋值
														          delta:{
																	  ops:[
																		  {
																			insert:_this.article.content
																		  }
																	  ]
																  }
															});
														}).exec();
														_this.$refs.listEditor.reInit(_this.article.content);
														_this.countText();
			...
			change(e) {
				uni.createSelectorQuery().select('.textarea').context((res) => {
				    this.editorCtx = res.context;
					this.article.content = JSON.stringify(e);
					this.article_changed = true;
				    this.editorCtx.setContents({//赋值
				          delta:{
							  ops:[
								  {
									insert:JSON.stringify(e)
								  }
							  ]
						  }
					});
