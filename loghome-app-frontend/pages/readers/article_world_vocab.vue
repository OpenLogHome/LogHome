<template>
	<view class="content" :class="readerSettings.theme" style="transition: all .5s;">
		<el-alert
			...
			type="info"
			close-text="知道了"
			:description="'经审核，本文' + article.warn_status + '，请酌情选读。'"
			show-icon
			v-show="article.warn_status && article.warn_status != 'None'"
			style="margin-bottom: 50rpx;"
			effect="dark"
			>
		</el-alert>
		<div class="tools" :class="{opened: settingsOpened}" @click.self="toolsOuterClicked"
		ref="tools">
			<div class="settings" :class="{opened: settingsOpened}" ref="settings">
				<div class="line">
					<div class="button" @click="changeFontSize(+1)">A+</div>
					<div class="button" @click="changeFontSize(-1)">A-</div>
					<div class="button" :class="{'selected':readerSettings.lineHeightMode == 0}" @click="changeLineHeight(0)">窄</div>
					<div class="button" :class="{'selected':readerSettings.lineHeightMode == 1}" @click="changeLineHeight(1)">中</div>
					<div class="button" :class="{'selected':readerSettings.lineHeightMode == 2}" @click="changeLineHeight(2)">宽</div>
					<div class="button" @click="gotoMenu">目录</div>
				</div>
				<div class="line">
					<div class="button blue theme" @click="changeTheme('blue')" :class="{'selected':readerSettings.theme == 'blue'}">天空蓝</div>
					<div class="button yellow theme" @click="changeTheme('yellow')" :class="{'selected':readerSettings.theme == 'yellow'}">原木黄</div>
					<div class="button green theme" @click="changeTheme('green')" :class="{'selected':readerSettings.theme == 'green'}">草原绿</div>
					<div class="button purple theme" @click="changeTheme('purple')" :class="{'selected':readerSettings.theme == 'purple'}">末地紫</div>
					<div class="button black theme" @click="changeTheme('black')" :class="{'selected':readerSettings.theme == 'black'}">虚空黑</div>
				</div>
				<div class="line">
					<button type="default" class="inTopBar" :class="[{enabled:article.article_chapter != firstArticleChapter},readerSettings.theme]" @click="changePage(-1)">
						上一章<img src="../../static/icons/icon_reader_pgup.png" alt="" class="pageChangeImg">
					</button>
					<button type="default" class="inTopBar" :class="[{enabled:article.article_chapter != lastArticleChapter},readerSettings.theme]"  @click="changePage(+1)">
		...
		<div class="topBar">
			<div class="left" @click="imgUploadVisible = true">
				<div class="pic" v-if="article.content.pic == undefined && article.title"> {{article.title.slice(0, 1)}} </div>
		...
		@tap="articleTapped" :class="readerSettings.theme">
			...
			<el-card class="box-card" v-for="(item, index) in article.content.attributes" style="margin-bottom: 20rpx;">
		...
		<button type="default" :class="[{enabled:article.article_chapter != firstArticleChapter},readerSettings.theme]" @click="changePage(-1)">
			上一章<img src="../../static/icons/icon_reader_pgup.png" alt="" class="pageChangeImg">
		</button>
		<button type="default" :class="[{enabled:article.article_chapter != lastArticleChapter},readerSettings.theme]"  @click="changePage(+1)">
			<img src="../../static/icons/icon_reader_pgdn.png" alt="" class="pageChangeImg">下一章
		</button>
		<el-drawer
		  :with-header="false"
						...
						IDBOpenDBRequest.onsuccess = function(e){
							...
							request.onsuccess = function (e) {
								...
								if(cursor){
									...
									result.is_draft = 0;
									if(result.novel_id == uid) _this.articles.push(result);
									cursor.continue();
								} else {
					...
					if(dp == +1){
						for(let i = 0 ; i < this.articles.length ; i ++){
							if(this.articles[i].article_chapter >= this.article.article_chapter + dp
								...
								uni.redirectTo({
									url: './article?isDirectJump=1&id=' + aid
								});
								return;
							}
						}
				...
				if(_this.settingsOpened){
					...
					this.jsBridge.ready(function(){
						_this.jsBridge.setOptions({
							...
							statusBarBlackText:(_this.readerSettings.theme=="black" ? false : true)
						});
					})
			...
			axios.get(this.$baseUrl + '/articles/get_article?id=' + uid).then((res) => {
				...
				if(dbStatus == "enabled"){
					...
					IDBOpenDBRequest.onsuccess = function(e){
						...
						// 通过事务来获取IDBObjectStore
						let store = transaction.objectStore('offlineArticleCache');
						let request = store.get(Number(uid));
						request.onsuccess = function( event) {
							if(request.result){
								_this.article = request.result;
								_this.article.title = request.result.article_title;
								_this.article.content = JSON.parse(request.result.article_content);
								_this.article.novel_id = request.result.novel_id;
								_this.article.article_chapter = request.result.article_chapter;
								_this.article.is_draft = 0;
								window.localStorage.setItem("ReaderHistory_" + _this.article.novel_id,_this.article.article_chapter);
								_this.getArticles(_this.article.novel_id);
								_this.$forceUpdate();
							} else {
								uni.showToast({
									title: "网络连接失败",
