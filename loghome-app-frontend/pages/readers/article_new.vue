	...
	<view style="height: 100%;">
		...
		v-if="menuShow" @touchend="closeMenu">
			...
			<view class="menu-top" :style="{height: `${statusBarHeight + 40}px`, top: itemShow ? 0 : `-100%`}" @touchend.stop>
				...
				<view class="head">
					<uni-icons type="left" class="backIcon" @tap="toback" size="24" color="#fff"></uni-icons>
				</view>
			</view>
			<!-- 菜单主体 -->
...
<script>
	import battery from '../../components/newReader/battery.vue'
	import virtualList from '../../components/newReader/virtualList.vue'
	import { traditionalized, simplized, dateToStr } from '../../lib/utils.js'
	...
	export default {
		...
		onLoad(option) {
			...
			axios.get(this.$baseUrl + '/library/get_novel_by_id?id=' + this.bookId, {}).then((res) => {
				this.bookInfo = res.data[0];
				if(option.article_id != undefined){
					let cycleTimer = setInterval(()=>{
						if(_this.directoryList.length > 0){
							clearInterval(cycleTimer);
							for(let i = 0 ; i < _this.directoryList.length; i ++){
								if(_this.directoryList[i].chapterId == option.article_id){
									_this.goToChapter(_this.directoryList[i].index);
									break;
								}
							}
						}
		...
		methods: {
			...
			setStorage() {
				...
				window.localStorage.setItem("ReaderHistory_" + this.bookId,this.curChapter.chapterIndex+1);
					...
					axios.get(this.$baseUrl + '/library/get_articles?id=' + uid, {}).then((res) => {
						for(let i = 0;i < res.data.length ; i ++){
							this.directoryList.push({
								index: i,
								chapterId: res.data[i].article_id,   //注意：这个chapterId用于获取章节内容而不是index
			...
			**/
			getOneChapter(chapterId, showLoading) {
				...
				return new Promise((resolve, reject) => {
					axios.get(this.$baseUrl + '/articles/get_article?id=' + chapterId).then((res) => {
						let article = res.data[0];
						let contents = article.content.split("\n");
						let article_content = "";
						
						for(let content of contents){
							article_content = article_content + `<p>${content}</p>`
						}
						...
						this.tmpChapter.text = `<p style="font-weight:bold">${article.title}</p>` + article_content  //模拟数据
