...
export default {
	...
	methods: {
		...
		clipboardCheckInit() {
			...
			if(window.jsBridge == undefined || !window.jsBridge.inApp) {
				return;
			}
			let lastClipboardData = "";
			const _this = this;
			async function checkClipboard() {
				let newClipboardData = await window.jsBridge.getClipboardData();
				if (newClipboardData != lastClipboardData) {
					lastClipboardData = newClipboardData;
					console.log("剪贴板发生变化：", newClipboardData);
					
					// 检测是否为原木口令格式
					_this.checkShareCode(newClipboardData);
				}
			}
			setInterval(checkClipboard, 3000);
			this.$bus.$on("clipboardChange", async (data) => {
				lastClipboardData = data;
			})
		},
		checkShareCode(clipboardText) {
			if (!clipboardText) return;
			
			const shareCodeRegex = /【原木社区】([A-Z0-9]{8})，/;
			const match = clipboardText.match(shareCodeRegex);
			...
			if (match && match[1]) {
				...
				// 解析口令，同时传递share_message
				this.parseShareCode(code, shareMessage);
			}
		},
		parseShareCode(code, shareMessage = '') {
			uni.showLoading({
				title: '解析中...'
			});
		...
		axios.post(this.$baseUrl + '/library/parse_share_code', {
			code: code
				...
				if (res.data.success) {
					...
					if (shareMessage) {
						shareData.share_message = shareMessage;
					}
					
					// 显示分享内容详情弹窗
					this.showShareContentDialog(shareData);
				}
				// 书籍口令解析失败，尝试解析帖子口令
		}).catch((error) => {
			// 尝试解析帖子分享口令
			axios.post(this.$baseUrl + '/community/posts/parse_share_code', {
				code: code
			}).then((res) => {
				uni.hideLoading();
				...
				uni.hideLoading();
				console.error('解析口令失败:', postError);
				// 网络错误时也不显示提示
			});
		});
		},
		...
		let content = `分享类型：${shareTypeText}\n分享内容：${shareData.share_content}\n分享者：${shareData.share_user_name || '匿名用户'}\n使用次数：${shareData.use_count}`;
			...
			uni.showModal({
				...
				cancelText: '关闭',
				success: (res) => {
					if (res.confirm && shareData.target_url) {
						// 跳转到目标页面
						this.navigateToTarget(shareData.target_url);
					}
				}
			});
		},
