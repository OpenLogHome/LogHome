		...
		methods: {
			...
			heartbeatInit() {
				...
				const checkIm = () => {
					let _this = this;
					// 2. 检查uni-im外部联登情况
					let uniStatus = sessionStorage.getItem("uniStatus");
					if (uniStatus == undefined) {
						_this.uniImLogin();
					} else {
						let {
							tokenExpired
			...
			uniImLogin() {
				...
				if (tk) tk = tk.tk;
				if (tk == null) return;
				axios.post(baseUrl + '/uni/get-uni-token', {
					...
					if (res.data.errCode == "uni-id-account-not-exists") {
						_this.uniImRegister();
					} else {
						console.log(res.data);
						uniImUtils.login(res.data.newToken);
						sessionStorage.setItem("uniStatus", "true");
					}
				}).catch(function(error) {
					console.log(error);
				})
			...
			uniImRegister() {
				...
				if (tk) tk = tk.tk;
				if (tk == null) return;
				axios.get(baseUrl + '/users/userprofile', {
					headers: {
					...
					console.log(res.data);
					axios.post(baseUrl + '/uni/uni-register', {
						clientInfo: uni.getSystemInfoSync(),
			...
			async uniInit() {
				...
				if (tk) tk = tk.tk;
				if (tk == null) return;
				uniImUtils.getMessageCb((msg) => {
					console.log("IM消息监听器监听到本机发送了消息：", msg);
					// 向原木通知系统同步消息
					axios.post(baseUrl + '/uni/handle_send_message', msg.data, {
						headers: {
							'Content-Type': 'application/json',
							'Authorization': 'Bearer ' + tk
						}
...
<style lang="scss">
	...
	* {
	...
	@font-face {
		...
		src: url('./static/iconfont/iconfont.woff2?t=1716951916053') format('woff2'),
			url('./static/iconfont/iconfont.woff?t=1716951916053') format('woff'),
			url('./static/iconfont/iconfont.ttf?t=1716951916053') format('truetype');
	}
